...
const BACKEND_URL =
  process.env.BACKEND_URL || "https://kcx21158-3001.inc1.devtunnels.ms";
...
const useAuth = () => {
  ...
  const login = async (userId, role, name) => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/v1/webrtc/auth`, {
        method: "POST",
...
const KYCWebRTCSystem = () => {
  ...
  const initializeWebRTC = async () => {
    try {
      ...
      // Initialize WebRTC with authentication
  ...
  const startSession = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/v1/webrtc/sessions`, {
        method: "POST",
        headers: {
      ...
      if (response.ok && data.success) {
        ...
        // Fallback - still start session for demo purposes
        setSessionStarted(true);
      }
    } catch (error) {
  ...
  const startRecording = async () => {
    try {
      ...
      if (stream && token) {
        ...
        // Notify backend about recording status
        await fetch(
          `${BACKEND_URL}/api/v1/webrtc/sessions/${sessionId}/recording`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const stopRecording = async () => {
    if (mediaRecorderRef.current && isRecording && token) {
  ...
  const handleChecklistSubmit = async (results) => {
    if (!token) return;

    try {
      const response = await fetch(
        `${BACKEND_URL}/api/v1/webrtc/sessions/${sessionId}/verify`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({ checklist: results }),
        }
      );
      ...
      if (response.ok && data.success) {
        ...
        console.error("Failed to submit verification:", data.message);
        // Fallback for demo
        setChecklistResults(results);
        setVerificationComplete(true);
      }
                  ...
                  <CardContent className="space-y-2">
                    ...
                    {sessionData && (
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant="outline">{sessionData.status}</Badge>
                      </div>
                    )}
                  </CardContent>
                </Card>
